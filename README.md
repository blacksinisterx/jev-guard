...
- **`.python-version` pins the runtime to 3.12.** Without it, Vercel's zero-config Python function detection picked 3.14 (its newest default) instead of the 3.12 the legacy `builds`-array path had used — and `pydantic-core` has no prebuilt wheel for 3.14 yet, so it tries to compile from Rust source and fails outright (PyO3 doesn't support 3.14 either). Pin the version explicitly rather than relying on whatever Vercel currently defaults to.

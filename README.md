...
- **`pyproject.toml` pins the runtime to `>=3.12,<3.13`.** Vercel's zero-config Python detection picked 3.14 regardless of a `.python-version` file (confirmed with build cache explicitly disabled, so it wasn't a caching artifact — Vercel's own docs say 3.12 is the default with no version file at all, which didn't match what was observed either). `pyproject.toml`'s `requires-python` is the more fully-documented mechanism; the explicit upper bound rules out `>=3.12` still matching 3.14. `vercel.json`'s `build.env.PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1` is a safety net on top, in case version pinning doesn't take effect either way — it's the exact workaround the Rust build error itself suggests, letting `pydantic-core` build against whatever Python version shows up instead of refusing to compile at all.

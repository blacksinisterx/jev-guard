...
- **`framework: null` in `vercel.json`, explicitly.** A dashboard-auto-detected Framework Preset can silently override `buildCommand`/`outputDirectory` even after they're set in `vercel.json` — the leading suspect for the root URL 404ing while `/api/*` worked fine. Setting `framework: null` forces `vercel.json` to be authoritative regardless of what the project's own Settings page shows.
- **Only `pyproject.toml` for the deployed function — no `requirements.txt` at the root or in `api/`.** A `.python-version` file was ignored outright (confirmed with build cache explicitly disabled), and `pyproject.toml`'s `requires-python` was *also* ignored as long as a `requirements.txt` sat next to it — dependency resolution apparently took the `requirements.txt` path and never consulted `pyproject.toml` for anything, version pin included. Removing `requirements.txt` for the deployed function entirely (kept in `backend/` for local dev/Docker, untouched) and listing dependencies directly in `pyproject.toml` is what finally made `requires-python` take effect.
- **`vercel.json`'s `build.env.PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1`** — the exact workaround the Rust build error itself suggested — is kept as a safety net even with the version pin working, since it's harmless when the pinned version already has a prebuilt wheel (no compilation happens at all in that case).
...
The decision log is in-memory by design (a demo dashboard doesn't need persistence — swap in SQLite/Postgres first if this ever needs to survive cold starts).
